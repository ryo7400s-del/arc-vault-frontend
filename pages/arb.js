...
async function askClaude({ usdcBalance, eurcBalance, curveRate, stablefxRate, recentTrades, mode }) {
  const spread = curveRate && stablefxRate ? Math.abs(curveRate - stablefxRate) : 0;
  const prompt = `You are an on-chain arbitrage AI agent on Arc Testnet (Chain 5042002).
Wallet: ${DEV_WALLET}
Gas paid in USDC natively (no ETH needed).

Current rates (USDC per 1 EURC):
  Curve  (AMM, pool ${ADDR.CURVE_POOL}): ${curveRate?.toFixed(6) ?? "fetching"}
  StableFX (RFQ, escrow ${ADDR.STABLEFX_ESCROW}): ${stablefxRate?.toFixed(6) ?? "fetching"}

Spread: ${(spread * 100).toFixed(4)}%
Balances: USDC=${usdcBalance}, EURC=${eurcBalance}
Mode: ${mode}
Recent trades: ${recentTrades.length ? recentTrades.slice(0,3).map(t=>`[${t.ok?"✓":"✗"}] ${t.dir} ${t.amount} → profit ${t.profit}`).join(" | ") : "none"}

Min profitable spread (Curve fee ~0.04% + gas ~$0.001): ~0.05%

Respond JSON only:
{"action":"TRADE"|"WAIT","direction":"BUY_EURC_ON_CURVE"|"SELL_EURC_ON_CURVE"|null,"amountUSDC":0,"expectedProfit":0,"confidence":0.0,"reason":"<15 words","slippageBps":30}`;
  ...
  try {
    ...
    return { action: "WAIT", reason: "AI error — holding", confidence: 0, amountUSDC: 0 };
  }
}
...
export default function ArbPage() {
  ...
  const fetchChainData = useCallback(async () => {
    ...
    try {
      ...
      setBalances({
        usdc: parseFloat(formatUnits(usdcRaw, 6)),
        eurc: parseFloat(formatUnits(eurcRaw, 6)),
      });
      ...
      setPoolInfo({
        curveRate:    invRate,
        poolLiqUsdc:  parseFloat(formatUnits(poolUsdc, 6)),
        poolLiqEurc:  parseFloat(formatUnits(poolEurc, 6)),
        fee:          Number(feeRaw) / 1e10 * 100,
      });
      setStablefxRate(invRate + (Math.random() - 0.5) * 0.002);
      setTick(t => t + 1);
    } catch (err) {
      log("オンチェーンエラー: " + err.message, "err");
    }
  ...
  const executeTrade = useCallback(async (direction, amountUSDC, slippageBps) => {
    ...
    try {
      ...
      log(`Approve ${isUsdcToEurc ? "USDC" : "EURC"} → Curve Pool…`, "trade");
      ...
      log(`Exchange ${amountUSDC} ${isUsdcToEurc?"USDC→EURC":"EURC→USDC"} minOut:${formatUnits(minDy,6)}`, "trade");
      ...
      const tradeProfit = isUsdcToEurc ? actualOut - amountUSDC : actualOut - amountUSDC;
      const tradeRec = {
        id: Date.now(), dir: direction, amount: amountUSDC,
        profit: tradeProfit.toFixed(4), gasUSDC: "~0.001",
        ok: receipt.status === "success",
        txHash: exchangeTx, ts: new Date().toLocaleTimeString(),
        block: receipt.blockNumber?.toString(),
      };
      setTrades(t => [tradeRec, ...t.slice(0, 49)]);
      if (receipt.status === "success") {
        setProfit(p => p + tradeProfit);
        log(`✓ ${exchangeTx.slice(0,14)}… profit:+${tradeProfit.toFixed(4)} USDC gas:USDC`, "ok");
      } else {
        log("✗ TX reverted", "err");
      }
    } catch (err) {
      log("取引エラー: " + (err.shortMessage || err.message), "err");
    }
    setExecuting(false);
    ...
    const id = setInterval(async () => {
      ...
      try {
        const decision = await askClaude({
          ...
          eurcBalance:  balances.eurc?.toFixed(2) ?? "—",
          curveRate:    poolInfo.curveRate,
        ...
        if (decision.action === "TRADE" && mode !== "manual" && decision.amountUSDC > 0)
          await executeTrade(decision.direction, decision.amountUSDC, decision.slippageBps ?? 30);
      } catch (err) { log("AI error: " + err.message, "err"); }
      setAnalyzing(false);
    }, 18000);
  ...
  useEffect(() => {
    ...
    fetchChainData();
    const id = setInterval(fetchChainData, 6000);
    return () => clearInterval(id);
  ...
  const hasOpportunity = spread > 0.05;
      ...
      <div className="min-h-screen bg-gray-950 text-gray-300 font-mono text-xs">
        ...
        {wrongChain && (
          <div className="bg-red-950 border-b border-red-800 px-4 py-2 text-red-300 text-xs">
            ⚠️ Arc Testnet (Chain ID: 5042002) に切り替えてください。現在: {chain?.name} ({chain?.id})
          </div>
        )}
        ...
        <div className="bg-gray-900 border-b border-gray-800 px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-gray-600 text-[10px] tracking-[0.2em]">EURC/USDC · ARC TESTNET · CURVE AMM</div>
            </div>
          </div>
            ...
            {["manual","conservative","balanced","aggressive"].map(m => (
              <button key={m} onClick={() => setMode(m)}
                className={`px-2 py-1 rounded text-[10px] uppercase tracking-widest border transition-colors
                  ${mode===m ? "border-blue-500 bg-blue-950 text-blue-400" : "border-gray-800 text-gray-600 hover:text-gray-400"}`}>
                {m}
              </button>
            ))}
            ...
            {isConnected && !wrongChain ? (
              <button onClick={() => setRunning(r => !r)}
                className={`px-4 py-1.5 rounded text-xs font-bold tracking-widest transition-colors
                  ${running ? "bg-red-950 text-red-400 hover:bg-red-900" : "bg-green-950 text-green-400 hover:bg-green-900"}`}>
                {running ? "■ STOP" : "▶ START"}
              </button>
            ) : (
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
                ...
                <div className="text-right">
                  ...
                  {poolInfo.poolLiqUsdc && (
                    <div className="text-gray-700 text-[10px]">
                      TVL: {poolInfo.poolLiqUsdc.toFixed(0)} / {poolInfo.poolLiqEurc?.toFixed(0)}
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-lg p-4">
              <div className="text-gray-700 text-[9px] tracking-widest uppercase font-bold mb-3">AI DECISION — CLAUDE</div>
              {!ai ? (
                <div className="text-gray-800 text-center py-8">
                  {running ? "初回分析待ち (18秒周期)…" : "START を押してください"}
                </div>
              ) : (
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    <span className={`px-2 py-0.5 rounded text-xs font-bold tracking-widest border
                      ${ai.action==="TRADE" ? "bg-yellow-950 border-yellow-800 text-yellow-400"
                  ...
                  {ai.action === "TRADE" && (
                    <div className="bg-gray-950 border border-gray-800 rounded p-3 space-y-1">
                      {[
                        ...
                        ["スリッページ",`${ai.slippageBps ?? 30} bps`,    "text-gray-400"],
                  ...
                  <div className="text-gray-600 text-[11px] italic leading-relaxed">"{ai.reason}"</div>
                  {mode === "manual" && ai.action === "TRADE" && (
                    <button onClick={() => executeTrade(ai.direction, ai.amountUSDC, ai.slippageBps ?? 30)}
                      disabled={executing || !isConnected}
                      className="w-full mt-2 py-2 bg-yellow-950 hover:bg-yellow-900 border border-yellow-800 text-yellow-400 rounded text-xs font-bold tracking-widest disabled:opacity-40 transition-colors">
                      ⚡ 手動実行
                    </button>
                  )}
                </div>
                ...
                <table className="w-full text-[11px]">
                  <thead>
                    <tr className="text-gray-700 border-b border-gray-800">
                      {["時刻","方向","金額","利益","ガス","TX Hash","Block","状態"].map(h => (
                  ...
                  <tbody>
                    {trades.map(t => (
                      <tr key={t.id} className="border-b border-gray-900">
                        ...
                        <td className="px-2 py-1 text-blue-500">{t.gasUSDC}</td>
                        <td className="px-2 py-1">
                          {t.txHash ? (
                            <a href={`https://testnet.arcscan.app/tx/${t.txHash}`} target="_blank" rel="noreferrer"
                               className="text-blue-700 hover:text-blue-500 underline">{t.txHash.slice(0,12)}…</a>
                          ) : "—"}
                        </td>
                        <td className="px-2 py-1 text-gray-700">{t.block ?? "—"}</td>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-lg p-4">
            ...
            <div className="h-36 overflow-y-auto space-y-0.5 leading-6">
              {logs.map((l, i) => (
